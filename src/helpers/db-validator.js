...
export const validarTokensHotelUser = async (req, res, next) => {
  try {
    const { userIdentifier, hotelToken } = req.body;
    const secret = process.env.SECRETORPRIVATEKEY;

    let user;

    if (mongoose.Types.ObjectId.isValid(userIdentifier)) {
    user = await User.findOne({ _id: userIdentifier });
    } else {
    ...
    let hotelPayload;
    try {
      hotelPayload = jwt.verify(hotelToken, secret);
    } catch (err) {
      return res.status(400).json({ success: false, msg: "Token de hotel inválido" });
    }
